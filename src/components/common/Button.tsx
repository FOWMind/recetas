...
export const Button = ({ className = '', as: As = 'button', variant = 'default', ...props }: ButtonProps) => {
	return (
		<As
			className={`inline-block rounded-full px-10 py-3 text-center font-medium ${variant && variants[variant]} disabled:opacity-50 disabled:bg-transparent dark:disabled:bg-transparent ${className}`}
			{...props}
		/>
	)
}
